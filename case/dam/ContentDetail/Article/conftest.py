...
from conf.confs import url_assetPage, asset_test_data, Host2, account_LZJ, password_LZJ, url_import_baidu, \
    url_assetGroupPage
...
def asset_init(login_init):
    ...
    try:
        ...
        while groupObject._is_visible(selector) is True:
...
@pytest.fixture(scope='class')
def detail_article_init(login_init, asset_init):
    """
    进入素材详情页
    """
    asset_init._go(url_assetPage)
    asset_init.upload_article('UI自动化测试-文章-lzj')
