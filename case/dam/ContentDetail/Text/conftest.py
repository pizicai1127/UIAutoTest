...
from conf.confs import url_assetPage, asset_test_data, Host2, \
    url_assetGroupPage, account_LZJ_008, password_LZJ_008
...
def login_init(browser: Browser):
    ...
    if os.path.exists(cookie_file):
        ...
        login.login(Host2, account_LZJ_008, password_LZJ_008)
        time.sleep(2)
    yield page
    try:
        # 保存登录验证cookie
...
def asset_init(login_init):
    ...
    try:
        ...
        if 'UI自动化测试' in assetObject._text_content(selector):
            assetObject.asset_hover()
            assetObject.asset_more_button()
            assetObject.asset_more_delete_content_text()
            print('-----数据清除完毕-----')
...
@pytest.fixture(scope='class')
def detail_text_init(login_init, asset_init):
    """
    进入素材详情页
    """
    asset_init._go(url_assetPage)
    asset_init.upload_text('UI自动化测试-纯文本-lzj')
    ContentDetail = ContentDetailPage(login_init)
    ContentDetail.into_detail_text_page()
    print('-----进入素材详情页-----')
    yield ContentDetail
